...
class H4ArgumentParser(HfArgumentParser):
    ...
    def parse(self) -> DataClassType | Tuple[DataClassType]:
        ...
        elif len(sys.argv) > 2 and (sys.argv[1].endswith(".yaml") or sys.argv[2].endswith(".yaml")):
            if sys.argv[1].endswith(".yaml"):
                output = self.parse_yaml_and_args(os.path.abspath(sys.argv[1]), sys.argv[2:])
            else:
                output = self.parse_yaml_and_args(os.path.abspath(sys.argv[2]), sys.argv[1:2] + sys.argv[3:])
        # parse command line args only
        else:
            output = self.parse_args_into_dataclasses()
        # output = self.parse_yaml_file("llm_dpo/config/config_full.yaml")
        if len(output) == 1:
            output = output[0]
        return output
...
class DataArguments:
    ...
    #     metadata={"help": "The number of processes to use for the preprocessing."},
    # )
    # packing: bool = field(default=True, metadata={"help": "use packing"})
